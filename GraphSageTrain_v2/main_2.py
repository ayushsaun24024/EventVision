...
class ImprovedGraphEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers=3, heads=4, dropout=0.15):
        super().__init__()
        ...
        self.skip_proj = nn.Linear(hidden_dim * (num_layers + 1), hidden_dim)
...
class EnhancedDualEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, embed_dim, num_layers=3, heads=4):
        super().__init__()
        self.article_encoder = ImprovedGraphEncoder(input_dim, hidden_dim, embed_dim, 
                                                     num_layers, heads)
    ...
    def forward(self, article_x, edge_index, caption_x):
        ...
        if self.use_cross_attn:
            article_refined, _ = self.cross_attention(
                article_embed.unsqueeze(1), 
                caption_embed.unsqueeze(1),
                caption_embed.unsqueeze(1)
            )
            article_embed = F.normalize(article_refined.squeeze(1), dim=-1)
...
def main():
    ...
    model = EnhancedDualEncoder(input_dim, HIDDEN_DIM, EMBED_DIM, num_layers=3, heads=4).to(DEVICE)
